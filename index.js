...
app.get('/dashboard', (req, res) => {
    res.send(`
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h1>🤖 Cannoh MD Dashboard</h1>
                    <p>Heroku Deployment Manager</p>
                </div>
                
                <div class="stats-grid">
                    <div class="stat-card">
                        <h3>Connection Status</h3>
                        <p class="${bot.isConnected ? 'connected' : 'disconnected'}">
                    ...
                    <div class="stat-card">
                        ...
                        <p>Session: ${bot.sessionId}</p>
                        <p>Prefix: ${bot.currentPrefix}</p>
                        <p>Mode: ${bot.currentMode}</p>
                    </div>
                ...
                <div class="features-grid">
                    ${Object.entries(bot.config.features).map(([feature, enabled]) => `
                ...
                <div style="text-align: center; margin-top: 40px;">
                    ...
                    <a href="/api/health" class="btn">❤️ Health</a>
                    <a href="/api/restart" class="btn" onclick="return confirm('Restart bot?')">🔄 Restart</a>
                    <a href="${bot.config.deployment.webUrl}/logs" class="btn">📝 Logs</a>
                </div>
            </div>
        </body>
        </html>
...
app.get('/api/health', (req, res) => {
    const health = {
        ...
        bot: {
            connected: bot.isConnected,
            sessionId: bot.sessionId,
            features: Object.values(bot.config.features).filter(Boolean).length
        },
        system: {
            uptime: process.uptime(),
            memory: process.memoryUsage(),
...
app.get('/api/session/backup', async (req, res) => {
    try {
        const success = await bot.backupSession();
        res.json({ success, message: success ? 'Session backed up' : 'Backup failed' });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});
...
app.get('/api/logs', (req, res) => {
    // Simple log viewer (in production, use proper log management)
    res.send(`
        <html>
        <body>
            <h1>📝 Logs</h1>
            <p>For detailed logs, run:</p>
            <pre>heroku logs --tail -a ${bot.herokuAppName || 'your-app-name'}</pre>
...
if (bot.herokuApiKey && bot.herokuAppName) {
    app.get('/api/heroku/dyno', async (req, res) => {
        try {
            const response = await fetch(`https://api.heroku.com/apps/${bot.herokuAppName}/dynos`, {
                headers: {
...
app.use((req, res) => {
    res.status(404).json({
        ...
        available_endpoints: [
            ...
            { path: '/qr', method: 'GET', description: 'QR code scanner' },
            { path: '/dashboard', method: 'GET', description: 'Bot dashboard' },
            { path: '/api/health', method: 'GET', description: 'Health check' },
            { path: '/api/status', method: 'GET', description: 'Bot status' }
        ]
    });
});
...
app.listen(PORT, () => {
    ...
    console.log(`🔗 Heroku: ${bot.config.deployment.webUrl}`);
    console.log(`📱 QR Code: ${bot.config.deployment.webUrl}/qr`);
    console.log(`📊 Dashboard: ${bot.config.deployment.webUrl}/dashboard`);
    
    // Log Heroku-specific info
    if (process.env.HEROKU_APP_NAME) {
        console.log('\n📋 Heroku Commands:');
        console.log(`📝 View logs: heroku logs --tail -a ${process.env.HEROKU_APP_NAME}`);
        console.log(`⚙️ Config vars: heroku config -a ${process.env.HEROKU_APP_NAME}`);
        console.log(`🔄 Restart: heroku restart -a ${process.env.HEROKU_APP_NAME}`);
...
process.on('SIGTERM', async () => {
    ...
    // Backup session before shutdown
    await bot.backupSession();
